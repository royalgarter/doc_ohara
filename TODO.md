...
8. [x] Full QASPER matrix (150 queries × 7 configs) + weight grid search → tuned (bm25 0.6, vector 1.0): Hits@10 30.0%→33.3%, MRR 0.179 ≥ vector-only. Results in paper §7.1.1. Tuned weights PERSISTED to DB env collection 2026-07-16 (`OHARA_BM25_WEIGHT=0.6`, `OHARA_VECTOR_WEIGHT=1.0`) after cross-corpus validation on MultiHop.
9. [~] Ingest MultiHop-RAG. **Smoke done (2026-07-12)**: first 100 docs ingested (`doc_pipeline/input/mhrag_smoke100/`), dates set, 60-query smoke matrix (15/type, `eval/multihop_smoke_queries.json`). Fixes found: (a) run_matrix doc matcher now maps gold titles→source_file via manifest (DB doc.title stores filename → old matcher scored 0%); (b) ingest-time embeddings untagged/wrong model → must run `scripts/backfill-embeddings.js --write` after EVERY ingest batch (3,912 re-embedded). Smoke results (100-doc corpus, saturated): vector_only best (H@10 100%, MRR 0.877); full 97.8%/0.764; temporal decay slightly HURTS temporal MRR (0.563 vs 0.608 no_temporal) — reversal vs pre-backfill read; null abstention full 46.7% vs vector 20%, bm25 93.3%. **FULL RUN DONE (2026-07-15)**: 609/609 ingested (~19.5M tokens ≈ $4.5 std; mhrag_0258 partial), embeddings 99.2% gemini-embedding-2, dates set. 500-query × 8-config matrix + tuned run (`eval/matrix_multihop_2026-07-15T15-52-49.json`, `...T17-29-24.json`). Headline: tuned full (bm25 0.6/vec 1.0) H@10 98.4%/MRR 0.812 = vector_only parity (QASPER tuning generalizes, default full 0.791); bm25 94.4%/0.727. **Bet 1a temporal: LOST** — no_temporal beats full on temporal slice (MRR 0.749 vs 0.736); decay hurts event-ordering queries; report as honest negative. **Bet 1b null abstention: WON** — corroboration abstains 45.6% on null queries (tuned 31.2%) vs no_corroboration top-k 0%, while keeping 91.5% Principal-hit on answerable; bm25's 96.8% abstention is degenerate (4.3% Principal-hit). Ablations all within ±0.5pp = noise, consistent with QASPER. NOTE: doc-level scoring (gold doc in top-10) ≠ MultiHop-RAG paper's evidence-level — not directly comparable to published numbers, caveat in paper.
10. [x] Generation eval (100 MultiHop queries, flash-lite gen + judge, `tests/eval/run_generation.js`): full_tuned 54% vs vector_only 57% — parity, within published retrieved-context band (0.44–0.56). Inference 88% / comparison ~43% / temporal ~34%; 41% "insufficient information" (paragraph-selection gap, not hallucination). Report `eval/generation_multihop_2026-07-16T01-56-41.json`.
