...
\item \textbf{Empirical Baseline Comparison:} We run LightRAG, reconfigured to OHARA's exact Gemini models, head-to-head on both QASPER and MultiHop-RAG -- addressing the absence of direct empirical comparison against cited graph-RAG systems.
...
Parameters tuned on QASPER transfer directly to MultiHop-RAG (Table~\ref{tab:multihop}), reaching Hits@10 and MRR parity while improving MAP (0.556 vs.\ 0.538). We also extended the LightRAG+Gemini baseline (Section~\ref{sec:eval71}) to the full 608-document MultiHop-RAG corpus, reaching 68.0\% answer-string recall (340/500 queries) under the same unranked-context caveat as the QASPER baseline -- directional evidence, not a controlled comparison to our document-level Hits@10. Empirical evaluation highlights three key insights:
...
\end{tabular}
\vspace{2pt}
\footnotesize{*BM25-only's 4.3\% P-hit reflects a near-empty Principal tier (few candidates ever corroborate); its 96.8\% abstention is a direct consequence of that emptiness, not discrimination -- read both asterisked cells together, not as independent signals. †Proxied as top-5. $^\ddagger$Same LightRAG+Gemini baseline as Table~\ref{tab:qasper}, run on the full 608-document MultiHop-RAG corpus (500 queries). $^\S$Not H@10: LightRAG's unranked context blob has no document ids, so we score answer-string presence (does the gold short answer appear in the context) instead of doc-level ranking -- a looser, non-comparable metric to this table's other columns.}
\end{table}
