...
- **2.3 Knowledge Graph Visualization** - 3D graph layouts (n3.js, GONE, VR graph tools), temporal graph visualization (Time-arc, Storyline). OHARA's sunburst-tunnel metaphor is, to our knowledge, novel on two counts: no prior work places document structure as concentric rings on time-anchored discs, and none uses an ontology sunburst as the cross-sectional coordinate system of a temporal tunnel — combining the 2D sunburst tradition (Stasko & Zhang, 2000, radial space-filling hierarchies) with 3D temporal graph layout so that topic columns become traceable through time.
...
- **4.11 Tier Classification** - Principal (≥2 phases, cross-doc or multi-doc, score ≥ 75th pctl), Integrity (Principal + verified neighbours with provenance), Explorer (frontier beyond Integrity, metadata-only). Grounded in Bates' Berrypicking + Pirolli's Information Foraging Theory.
...
Table 1 reports **preliminary development-corpus figures** (50 mixed PDF/DOCX documents, median 4,200 words; single run, one machine), retained to show the pipeline's per-stage cost *shape*; the authoritative cost measurements are the QASPER-200 and MultiHop-609 runs reported below, which supersede these numbers. Note the per-document token totals differ by design: the development documents were short (~4k words, few chunks), whereas QASPER papers average 24 KB of markdown across many chunks, each structured with repeated context — hence ~8k versus ~53k tokens per document across the two corpora.
...
**Figures**: publication-quality captures (3840×2160, fitted camera, MultiHop-RAG corpus, 25 documents spread across four monthly buckets at month resolution) in `eval/viz/pub_{doc,type,sumo}_25docs.png` (three color modes, whole-scene fitted view with sunburst guide rings, category labels, timeline ticks, and decay auras) and `eval/viz/pub_tunnel_oblique_25docs.png` (oblique tunnel-axis view). Capture script: `tests/eval/shoot_pub.js`. Implementation note: `Box3.expandByObject` ignores `InstancedMesh` instance matrices, so both the in-app fitted camera and the capture script compute the scene bounding box from per-instance positions (in-app fix applied).
...
- **7.1 Retrieval Quality** - **Measured** (Sections 7.1.1–7.1.2): config matrix of BM25-only, vector-only, full pipeline, per-phase ablations, and tuned weights on two standard corpora — Hits@4/10, MRR@10, MAP@10, gold-Recall@10, Principal-tier hit rate.
- **7.2 Tier Explainability** - For each Principal-tier node, analyze `contributions` array provenance. Original hypothesis (multi-phase corroboration > single-phase retrieval) **measured and revised**: ranking lands at parity, corroboration operates over two signal families (`bm25∩vector`), and the constraint's payoff is abstention on unanswerable queries (Sections 7.1.1–7.1.2, 4.13.4).
...
Scene rebuild time and memory grow sub-linearly in node count (17× more nodes → ~1.7× rebuild time, ~1.5× heap), confirming that the `InstancedMesh` batching keeps per-node overhead marginal; the practical ceiling is visual clutter and fill-rate, not geometry submission. (Benchmark: `tests/eval/bench_viz.js`.) Caveat: five node-count points, each a single run — no repeated trials, no reported variance; the sub-linear description fits these five samples, not a validated scaling law.
...
| LightRAG (Gemini)† | n/a | 34.7%‡ | n/a | n/a | n/a | n/a |

\* Principal proxied as plain top-5 when the corroboration constraint is disabled.
† LightRAG (Guo et al., 2024), reconfigured to use the same `gemini-2.5-flash-lite` / `gemini-embedding-2` models as OHARA for an architecture-only comparison, ingested on the identical 200-doc QASPER corpus.
‡ Not a ranked Hits@10: LightRAG's hybrid-mode query returns a single unranked context blob (~130–140K chars — entities, relations, and chunks with no top-k cutoff), scored as a binary hit if any gold-evidence snippet overlaps the blob. MRR/MAP/Gold-Recall/Principal-hit are undefined without rank order. Because the comparison point (OHARA) is constrained to a ranked top-10, while LightRAG is not constrained at all, this number likely *understates* OHARA's context-efficiency advantage rather than overstating OHARA's performance.
...
† Principal proxied as plain top-5 when the corroboration constraint is disabled.
†† LightRAG's columns are dashes because its scoring protocol doesn't map onto this table's doc-level ranking metrics; see the LightRAG baseline paragraph below for its number and caveats.

**Baseline comparison (LightRAG).** Extending the same LightRAG (Guo et al., 2024) + Gemini baseline used for QASPER (Section 7.1.1) to the full 608-document MultiHop-RAG corpus (one article permanently excluded — a Gemini safety-filter block on its content prevented extraction regardless of retries), queried in hybrid mode over all 500 stratified queries. Because LightRAG's response is again an unranked, uncapped context blob rather than per-document nodes, OHARA's document-level Hits@10 metric doesn't apply directly; instead we score **answer-string presence** — does the query's short gold answer (e.g. "Google") appear literally in the retrieved context — a standard, weaker-guarantee RAG recall proxy. LightRAG+Gemini reaches **68.0% answer-recall (340/500)**. This is not comparable in kind to OHARA's 96.5–98.4% document-level Hits@10 above (a looser containment check against a much larger unranked context vs. a strict top-10 document match), so we report it as directional evidence of baseline competence, not a controlled ranking comparison. Scorer: `baseline/lightrag/score_multihop.py`.

Four findings, two of which decide claims made earlier in the paper.

**First, the QASPER-tuned weights generalize across corpora.** The (bm25 0.6, vector 1.0) setting tuned on academic Q&A (Section 7.1.1) transfers unchanged to news: MRR improves from 0.791 to 0.812 and Hits@4 from 89.6% to 91.7%, matching vector-only on Hits@10 (98.4% both) and *slightly exceeding* it on MRR (0.812 vs. 0.811, a marginal win rather than a strict tie) and on MAP (0.556 vs. 0.538) — while retaining tiered, provenance-carrying output. We flag the wording deliberately: on QASPER "parity" describes a tie reached only via post-hoc tuning (see the tuned-on-test caveat, Section 7.1.1); here it undersells a small but real improvement. The lexical-first default prior, not the fusion architecture, was the gap on both corpora.

**Second, corroboration-gated abstention is the pipeline's one differentiated quantitative win — and its scope is abstention behavior, not ranking quality.** The controlled comparison is full vs. −corroboration, which share identical retrieval and differ only in the Principal-tier rule: the corroboration constraint abstains on **45.6% of unanswerable queries (57/125) versus 0.0% for a plain top-k tier**, while holding 91.5% Principal-hit on answerable queries. At n=125 null queries this carries roughly ±8–9pp sampling uncertainty at typical confidence levels. Note what it does not change: full vs. −corroboration differ by only 0.3pp Hits@10 (96.5% vs. 96.8%) and 0.5pp Principal-hit (91.5% vs. 92.0%) — both within the pipeline's ablation noise band — so the corroboration constraint's entire measured payoff is the abstention behavior, not a ranking-quality filter. A top-k cut structurally cannot abstain — it always emits k results; requiring ≥2 independent phase signals gives the tier a natural empty state. Vector-only reaches just 13.6% abstention. Tuning trades some abstention away (31.2%): down-weighting BM25 weakens one of the two corroborating signals, an explicit precision/abstention trade-off.
...
- **8.1 Limitations** - Single LLM (Gemini) for structuring + enrichment → prompt sensitivity. All reported evaluations ran on full ArangoDB; the in-memory simulator is a development mode only and its AQL coverage is limited. Visualization scalability: rendering is measured to 12k nodes, but visual clutter grows past ~50 selected documents (Section 8.2g). User study not yet run — protocol, task sheets, gold answers, and the flat-list comparison interface are prepared (Section 6).
...
- **8.3 Threats to Validity** - *Internal*: fusion weights were tuned on a 75-query QASPER subset; although the setting transferred unchanged to MultiHop-RAG (Section 7.1.2), two corpora do not establish domain-general defaults, and LLM-dependent stages (structuring, SUMO tagging, edge enrichment) introduce non-determinism that caching only partially controls. *External*: both evaluation corpora (200 academic papers, 609 news articles) are clean born-digital text; generalization to noisy OCR or poorly structured documents is untested, and one corpus per genre cannot separate genre effects from corpus effects. *Construct*: document-level (MultiHop) and paragraph-substring (QASPER) gold matching are proxies with different hardness — cross-protocol absolute numbers are not comparable (comparability notes, Sections 7.1.1–7.1.2) — and ranking metrics may not capture the exploratory-search benefits the tier system and visualization target; the planned user study addresses this. *Conclusion*: single-run latency measurements on one machine; variance not yet reported. The LightRAG baseline comparison (Section 7.1.1) uses a different scoring protocol than OHARA's own metrics — unranked context-blob overlap versus ranked top-k — so it is directional evidence of parity, not a controlled apples-to-apples ranking comparison; HippoRAG and a rank-aware LightRAG harness remain future work. Additionally, the preliminary smoke run (Section 7) exposed a fusion-score calibration issue: without per-phase normalization, BM25 magnitude dominates the weighted sum, which would silently reduce the multi-phase architecture to lexical search; all reported fusion and tier results must therefore be interpreted relative to the normalization scheme in use.
